...
void ImageData::ResizeTo(ImageData* pSource, int iNewWidth, int iNewHeight, bool bJustSampleOnePixel) {
	...
	for (int y = 0; y < m_iHeight; y++) {
		...
		if (bJustSampleOnePixel) { // Just sample one of the pixels - not a good thing!
			dX[0] = dY[0] = 1.0;
			dX[1] = dY[1] = 0.0;
		}
...
void ImageData::RenderImage(SDL_Surface* pTarget,
	...
	if (iXTarget < 0) { // Then trim
		iWidth += iXTarget;
		iXSource -= iXTarget;
		iXTarget = 0;
	}

	if ((iXTarget + iWidth) >= pTarget->w) { // Then trim
		iWidth = pTarget->w - iXTarget;
	}

	if (iYTarget < 0) { // Then trim
		iHeight += iYTarget; // Decrease height
		iYSource -= iYTarget; // Increase start point
		iYTarget = 0;
	}

	if ((iYTarget + iHeight) >= pTarget->h) { // Then trim
