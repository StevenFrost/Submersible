...
void BaseEngine::DrawThickLine(double fX1, double fY1, double fX2, double fY2,
	...
	if (iThickness < 2) { // Go to the quicker draw function
...
void BaseEngine::DrawPolygon(
	...
	for (int i1 = 0; i1 < iPoints; i1++) {
		...
		if (fAngle2 < fAngle3) { // Then points are in clockwise order so central one can be eliminated as long as we don't
			// fill an area that we shouldn't
			bool bPointIsWithinTriangle = false;
			if (iPoints > 3) { // Need to check that there isn't a point within the area - for convex shapes
			...
			if (!bPointIsWithinTriangle) {// If not then try the next position
