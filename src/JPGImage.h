...
class ImageData {
public:
	ImageData() : m_iWidth(0), m_iHeight(0), m_aiPixels(NULL) {}
	~ImageData() {
		if (m_aiPixels != NULL) {
			delete[] m_aiPixels;
		}
	...
	void RenderImage(SDL_Surface *pTarget, int iXSource, int iYSource, int iXTarget, int iYTarget, int iWidth, int iHeight);
	void RenderImageWithMask(SDL_Surface *pTarget, int iXSource, int iYSource, int iXTarget, int iYTarget, int iWidth, int iHeight);
...
class ImageSurface {
public:
	ImageSurface() : m_pSurface(NULL), isLoaded(false) {}
	~ImageSurface() {
		if (m_pSurface != NULL) {
